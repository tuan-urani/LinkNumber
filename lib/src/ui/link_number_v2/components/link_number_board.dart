...
import 'package:flow_connection/src/ui/link_number_v2/components/link_number_result_overlay.dart';
import 'package:flow_connection/src/ui/link_number_v2/components/link_number_v2_ball.dart';
import 'package:flow_connection/src/ui/link_number_v2/interactor/link_number_merge_timing.dart';
...
class _LinkNumberBoardState extends State<LinkNumberBoard>
  ...
  double _tileScale({
    ...
    // Keep core GIF size fixed in V2 preview.
    return 1.0;
  }
          ...
          builder: (_, child) {
            ...
            for (int index = 0; index < _burstPath.length; index++) {
              ...
              final burstValue = index < _burstPathValues.length
                  ...
                  : 2;
              final burstColor = linkNumberV2ColorForValue(burstValue);
...
class _CellTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ballState = destroyProgress > 0
        ? LinkNumberV2BallState.destroy
        : selected && !isSwapAnchor
        ? LinkNumberV2BallState.selected
        : LinkNumberV2BallState.idle;
    final ballKey = ballState == LinkNumberV2BallState.destroy
        ? destroyingAnimationKey
        : ValueKey<String>('tile_ball_${value}_${selected}_$isSwapAnchor');
                      ...
                      Positioned.fill(
                        child: DecoratedBox(
                          decoration: BoxDecoration(
                            ...
                            boxShadow: <BoxShadow>[
                              BoxShadow(
                                color: linkNumberV2ColorForValue(
                                  value,
...
class _PathPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int index = 0; index < path.length - 1; index++) {
      ...
      final startColor = linkNumberV2ColorForValue(startValue);
