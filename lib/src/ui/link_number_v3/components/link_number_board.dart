...
import 'package:flow_connection/src/ui/link_number_v3/interactor/link_number_merge_timing.dart';
...
class LinkNumberBoard extends StatefulWidget {
  const LinkNumberBoard({
    ...
    this.isWatchRewardAdLoading = false,
...
class _LinkNumberBoardState extends State<LinkNumberBoard>
  ...
  void initState() {
    ...
    _tutorialGuideController = AnimationController(
      vsync: this,
    ...
    _syncTutorialGuideAnimation();
  }

  @override
  void didUpdateWidget(covariant LinkNumberBoard oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.enableDropCascade && !widget.enableDropCascade) {
      _dropCascadeController.stop();
      if (mounted && (_isDropCascadeRunning || _dropMotions.isNotEmpty)) {
        setState(() {
  ...
  void dispose() {
    ...
    _tutorialGuideController.dispose();
    final selectedPool = _selectedSfxPool;
  ...
  void _handleBoardTapDown(
    ...
    if (selectedSkill == null) {
      // Seed drag path immediately on pointer-down so hold-to-select gets
      // instant visual/audio feedback without waiting for pan threshold.
      widget.onPanStart(localPosition, boardSize);
      return;
    }
  ...
  void _syncTutorialGuideAnimation() {
    final shouldAnimate = widget.tutorialFocusCell != null;
    if (shouldAnimate) {
      if (!_tutorialGuideController.isAnimating) {
        _tutorialGuideController.repeat(reverse: true);
      }
      return;
    }

    if (_tutorialGuideController.isAnimating) {
      _tutorialGuideController.stop();
    }
    if (_tutorialGuideController.value != 0) {
      _tutorialGuideController.value = 0;
    }
  }
  ...
  void _startReleasePathLineFade(List<LinkNumberCell> path) {
    _releasePathLineController.stop();
    _releasePathLineController.value = 1;
    setState(() {
  ...
  void _handlePanEnd() {
    ...
    if (shouldPlayResolve) {
      ...
      _startReleasePathLineFade(path);
  ...
  void _deriveTransientEffects({
    ...
    if (isValidMergeTransition) {
      final destroyedCount = math.max(1, oldSnapshot.activePath.length - 1);
      _playMergeDestroySfx(destroyedCount: destroyedCount);
  ...
  void _deriveSelectionFeedback({
    required LinkNumberSnapshot oldSnapshot,
    required LinkNumberSnapshot newSnapshot,
  }) {
    if (newSnapshot.selectedSkill != null || newSnapshot.isGameOver) {
      return;
    }
    final didAddSelectionCell =
        newSnapshot.activePath.length > oldSnapshot.activePath.length &&
        newSnapshot.activePath.isNotEmpty;
    if (!didAddSelectionCell) {
      return;
    }
  ...
  Future<void> _playSinglePopCellSfx() async {
    await _playSelectedSfx();
  }

  Future<void> _playPoolOneShot(
    AudioPool? pool, {
  ...
  double _tileScale({
    ...
    if (selected) {
      tileScale -= (0.03 - (0.008 * chainPulse));
    }
    ...
    if (_poppingCells.contains(cell)) {
      ...
      tileScale += 0.1 * remaining;
    }

    if (destroyProgress > 0) {
      tileScale *= (1 - (0.25 * destroyProgress));
    }

    return tileScale.clamp(0.65, 1.22);
  }
  ...
  Widget _buildTutorialTapGuideOverlay({
    required Size boardSize,
    required int rows,
    required int columns,
  }) {
    final focusCell = widget.tutorialFocusCell;
    if (focusCell == null) {
      return const SizedBox.shrink();
    }
    ...
    final cellWidth = boardSize.width / columns;
    final cellHeight = boardSize.height / rows;
    final guideSize = math.min(cellWidth, cellHeight) * 0.76;

    return Positioned.fill(
      child: IgnorePointer(
        child: AnimatedBuilder(
          animation: _tutorialGuideController,
          builder: (_, child) {
            final pulse = Curves.easeInOut.transform(
              _tutorialGuideController.value,
            );
            final haloScale = 0.92 + (0.12 * pulse);
            final handRise = 2 + (5 * pulse);
            return Stack(
              children: <Widget>[
                Positioned(
                  left: center.dx - (guideSize / 2),
                  top: center.dy - (guideSize / 2),
                ...
                Positioned(
                  left: center.dx + (guideSize * 0.08),
                  top: center.dy - (guideSize * 0.82) + handRise,
                  child: DecoratedBox(
                    decoration: BoxDecoration(
                    ...
                    child: Padding(
                      ...
                      child: SvgPicture.asset(
                        AppAssets.tapTutorialSvg,
                        width: guideSize * 0.42,
                        height: guideSize * 0.42,
                  ...
                  Positioned(
                    ...
                    child: SizedBox(
                      ...
                      child: LinkNumberV3Tile(
                        ...
                        destroyProgress: impactProgress,
                      ),
                    ),
                  ),
          ...
          builder: (_, child) {
            ...
            for (int index = 0; index < _burstPath.length; index++) {
              ...
              final burstValue = index < _burstPathValues.length
                  ...
                  : 2;
              final burstColor = linkNumberV3TileColorForValue(burstValue);
              final center = _cellCenter(
                        ...
                        children: <Widget>[
                          ...
                          if (widget.showResultOverlay && snapshot.isGameOver)
                            LinkNumberResultOverlay(
                              ...
                              isWatchRewardAdLoading:
                                  widget.isWatchRewardAdLoading,
                            ),
                        ],
                      ),
                      builder: (_, child) {
...
class _CellTile extends StatelessWidget {
  static const double _tileContentFactor = 0.92;
  ...
  Widget build(BuildContext context) {
    ...
    final dynamicLift = selected ? (-0.8 * chainPulse) : 0.0;
    final tileOpacity = hidden ? 0.0 : (1 - destroyProgress).clamp(0.0, 1.0);
    final tileState = destroyProgress > 0
        ? LinkNumberV3TileState.destroy
        : selected && !isSwapAnchor
        ? LinkNumberV3TileState.selected
        : LinkNumberV3TileState.idle;
    final tileKey = tileState == LinkNumberV3TileState.destroy
        ? destroyingAnimationKey
        : ValueKey<String>('tile_cell_${value}_${selected}_$isSwapAnchor');

    return DecoratedBox(
      decoration: BoxDecoration(
        border: showCellBorder
            ? Border.all(
                color: AppColors.colorF59AEF9.withValues(alpha: 0.5),
                width: 0.8,
              )
            : null,
      ),
                ...
                child: Stack(
                  ...
                  children: <Widget>[
                    LinkNumberV3Tile(
                      ...
                      selectionPulse: selected ? chainPulse : 0,
                    ),
                    if (isPopping)
                      Positioned.fill(
                        child: DecoratedBox(
                          decoration: BoxDecoration(
                            ...
                            boxShadow: <BoxShadow>[
                              BoxShadow(
                                color: linkNumberV3TileColorForValue(
                                  value,
...
class _PathPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int index = 0; index < path.length - 1; index++) {
      ...
      final startColor = linkNumberV3TileColorForValue(startValue);
