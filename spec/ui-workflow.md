...
### 1. Description
Goal: Show a focused landing screen that lets users choose play mode before entering Link Number V3.
Features:
- Splash routes to this menu screen as the first destination.
- Displays the main logo, level card, mode selector, and a single `PLAY` CTA.
- Supports two modes: `LEVEL` and `ENDLESS`.
- Shows dynamic info below mode selector:
  - Level mode: current level.
  - Endless mode: best tile.
- Navigates to `LinkNumberV3Page` with route argument `playMode`.

### 2. UI Structure
- Screen: `GameMenuPage`
- Components:
- `GameMenuTopBadges`: coin badge section.
- `_GameMenuLevelCard`: classic level card.
- `_GameMenuModeSelector`: segmented selector for `LEVEL` and `ENDLESS`.
- `_GameMenuModeInfo`: contextual line (`Level` or `Best Tile`).
- `GameMenuPlayButton`: open gameplay with selected mode.

### 3. User Flow & Logic
1. User opens app and sees splash.
2. After splash delay, app navigates to `GameMenuPage`.
3. User selects `LEVEL` or `ENDLESS`.
4. User taps `PLAY`.
5. App pushes `LinkNumberV3Page` with route argument:
   - `playMode: level`
   - `playMode: endless`

### 4. Key Dependencies
- `GameMenuController`: mode state + route argument dispatch.
- `LinkNumberPlayMode`: mode enum shared with V3.
- `AppPages`: route registration and navigation.
...
### 1. Description
Goal: Provide the main gameplay screen with shared V3 rendering and two rule branches: `Level` and `Endless`.
Features:
...
- Persists `endlessBestTile` and updates it when player creates a higher value tile.
...
### 3. User Flow & Logic
1. User opens V3 from menu `PLAY` with selected `playMode`.
...
### 4. Key Dependencies
- `LinkNumberEngine`: shared core rules + mode branching (`level`/`endless`).
- `LinkNumberController`: route-argument mode setup, tutorial gating, and action orchestration.
- `GameProgressManager` + `AppShared`: persistence for level progress and `endlessBestTile`.
